...
class _ModalContentState extends State<ModalContent> {
  ...
  Widget build(BuildContext context) {
    return CustomBottomSheet(
      height: 500.0,
      child: Expanded(
        child: StoreConnector<AppState, ProtocolRepresentativesViewModel>(
          converter: (store) => store.state.protocolRepresentativesState,
          distinct: true,
          builder: (_, state) {
            return state.searchingOtherOrgs == true
                ? const Center(
                ...
                : state.searchError == true
                    ...
                    : state.otherList.isNotEmpty
                        ? ListView.builder(
                            padding: EdgeInsets.zero,
                            controller: _scrollController,
                            itemCount: state.otherList.length,
                            itemBuilder: (_, int index) {
                              return CustomSelectRow(
                                index: index,
                                item: SelectObject(id: state.otherList[index].id.toString(), title: state.otherList[index].name),
                                last: index == (state.otherList.length - 1),
                                checkedId: (state.selectedOtherOrg != null) ? state.selectedOtherOrg!.split('_').first : null,
                                callback: (__, _) {
                                  StoreProvider.of<AppState>(context).dispatch(
                                    UpdateOtherOrgChoice(selectedOtherOrg: '${state.otherList[index].id}_${state.otherList[index].name}'),
                                  );
